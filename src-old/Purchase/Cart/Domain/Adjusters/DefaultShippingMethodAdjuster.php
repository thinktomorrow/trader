...
class DefaultShippingMethodAdjuster implements Adjuster
{
    ...
    {
        if($cart->shipping()->hasMethod()) return;
    ...
    {
        if(!$method = config('optiphar.cart-defaults.method')) return;
    ...
    private function firstApplicable(Cart $cart)
    {
        ...
        if($applicableShippingMethods->isEmpty()) return;
    ...
    private function fromCustomer(Cart $cart)
    {
        ...
        if(! $lastOrder = $customer->orders->where('statuspayment', PaymentStatus::RECEIVED)->sortBy('mtime')->last()) return;
