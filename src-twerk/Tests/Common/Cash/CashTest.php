...
class CashTest extends TestCase
{
    ...
    public function by_default_currency_code_is_used_as_symbol()
    {
        ...
        $this->assertEquals('AMD 1,20', $cash->toLocalizedFormat(Locale::default()));
    }
    ...
    public function it_can_get_percentage_with_specificity_of_2_decimals()
    {
        ...
        $this->assertEquals(Percentage::fromFloat(45.83), $cash->asPercentage(Money::EUR(120), 2));

        // Percentage can be rounded off
        $this->assertEquals(Percentage::fromInteger(46), $cash->asPercentage(Money::EUR(120), 0));
    }
    ...
    public function it_can_add_percentage_of_amount()
    {
        ...
        $this->assertEquals(120, Cash::from($money)->addPercentage(Percentage::fromInteger(20))->getAmount());
    }
    ...
    public function it_can_get_a_tax_percentage_of_gross_amount()
    {
        ...
        $this->assertEquals(100, Cash::from($money)->subtractTaxPercentage(Percentage::fromInteger(20))->getAmount());

        $money = new Money(1530, new Currency('EUR'));
        $this->assertEquals(1275, Cash::from($money)->subtractTaxPercentage(Percentage::fromInteger(20))->getAmount());

        $money = new Money(1200, new Currency('EUR'));
        $this->assertEquals(960, Cash::from($money)->subtractTaxPercentage(Percentage::fromFloat(25.0))->getAmount());
    }
}
