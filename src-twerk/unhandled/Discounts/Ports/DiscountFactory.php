...
namespace Thinktomorrow\Trader\Purchase\Discounts\Domain\Ports;
...
use Thinktomorrow\Trader\Purchase\Discounts\Domain\Conditions\BrandWhitelist;
use Thinktomorrow\Trader\Purchase\Discounts\Domain\Conditions\CategoryWhitelist;
use Thinktomorrow\Trader\Purchase\Discounts\Domain\Conditions\ConditionKey;
use Thinktomorrow\Trader\Purchase\Discounts\Domain\Conditions\ItemBlacklist;
use Thinktomorrow\Trader\Purchase\Discounts\Domain\Conditions\ItemWhitelist;
use Thinktomorrow\Trader\Purchase\Discounts\Domain\Conditions\MinimumItems;
...
class DiscountFactory
{
    ...
    private function conditions(Promo $promo, int $orderId = null): array
    {
        ...
        foreach ($promo->getRulesAsArray() as $key => $rule) {
            ...
            if ($rule instanceof IsCombinable) {
                continue;
            }
            if ($rule instanceof PromoGroup) {
                continue;
            }
    ...
    private function injectConditionsToMinimumItems(array $conditions): void
    {
        foreach ($conditions as $i => $condition) {
            if (! $condition instanceof MinimumItems) {
                continue;
            }
