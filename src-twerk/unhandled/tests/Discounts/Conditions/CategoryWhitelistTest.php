...
class CategoryWhitelistTest extends TestCase
{
    use OptipharDatabaseTransactions, CartHelpers;

    /** @test */
    public function category_whitelist_passes_if_no_whitelist_is_enforced()
    {
        $condition = new CategoryWhitelist([]);
    ...
    /** @test */
    public function category_whitelist_passes_if_given_item_is_in_whitelist()
    {
        $condition = new CategoryWhitelist(['123456']);
        $cart = Cart::empty(CartReference::fromString('xxx'));
        $cart->items()->add(CartFactory::makeItem(['category_ids' => [123456]]));

        $this->assertTrue($condition->check($cart, $cart->items()->first()));
    }

    /** @test */
    public function category_whitelist_does_not_pass_if_given_item_is_not_in_whitelist()
    {
        $condition = new CategoryWhitelist(['123456']);
        $cart = Cart::empty(CartReference::fromString('xxx'));
        $cart->items()->add(CartFactory::makeItem(['category_ids' => [99999]]));

        $this->assertFalse($condition->check($cart, $cart->items()->first()));
    }

    /** @test */
    public function category_whitelist_is_applied_if_given_item_is_in_whitelist()
    {
        $this->setUpDatabase();
        $this->provideCartData();

        $product = ProductFactory::create(['slug:nl' => 'xxx', 'gross_amount' => 500,])
                                ->withBrand()
                                ->withCategory(['id' => 123456])
                                ->toProductRead();
    ...
    /** @test */
    public function category_whitelist_is_not_applied_if_given_item_is_not_in_whitelist()
    {
        $this->setUpDatabase();
        $this->provideCartData();
