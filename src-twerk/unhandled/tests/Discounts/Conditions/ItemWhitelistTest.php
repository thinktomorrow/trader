...
class ItemWhitelistTest extends TestCase
{
    ...
    {
        $condition = new ItemWhitelist(['123456']);
        $cart = Cart::empty(CartReference::fromString('xxx'));
    ...
    public function item_discount_does_not_pass_if_given_item_is_not_in_whitelist()
    {
        $condition = new ItemWhitelist(['123456']);
        $cart = Cart::empty(CartReference::fromString('xxx'));
        $cart->items()->add(CartFactory::makeItem(['product_id' => 999999]));

        $this->assertFalse($condition->check($cart, $cart->items()->first()));
    }

    /** @test */
    public function item_discount_is_applied_if_given_item_is_in_whitelist()
    {
        $this->setUpDatabase();
        ...
        PromoFactory::createOrderPromo(['discount' => new DiscountPercentage(10)], ['ProductWhitelist' => new ProductWhitelist([$product->id], [], [])]);
    ...
    /** @test */
    public function item_discount_is_not_applied_if_given_item_is_not_in_whitelist()
    {
        $this->setUpDatabase();
        $this->provideCartData();
