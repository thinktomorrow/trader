...
class DiscountCollection implements \ArrayAccess, \Countable, \IteratorAggregate
{
    private $discounts = [];
    ...
    {
        if( !isset($this->discounts[$id]) ) return null;
    ...
    {
        $this->discounts[$discount->id()->get()] = $discount;

        return $this;
    }

    public function addMany($discounts): self
    {
        // If single item is passed, convert it to an array
        if($discounts instanceof Discount) $discounts = [$discounts];
    ...
    public function offsetSet($offset, $value)
    {
        ...
        {
            throw new \InvalidArgumentException('Adding discount to order requires an instance of '. Discount::class.' to be passed. '. gettype($value). ' given.');
        }

        if($offset != $value->id())
        {
            throw new \InvalidArgumentException('Key must be set as the discount id value. '. $offset. ' is given while ' . $value->id() .' was expected.');
        }
