...
class Period extends BaseCondition implements Condition, SaleCondition
{
    ...
    protected function validateParameters(array $parameters)
    {
        if (isset($parameters['start_at']) && !$parameters['start_at'] instanceof \DateTime) {
            throw new \InvalidArgumentException('SaleCondition value for start_at must be instance of DateTime.');
        }

        if (isset($parameters['end_at']) && !$parameters['end_at'] instanceof \DateTime) {
            throw new \InvalidArgumentException('SaleCondition value for end_at must be instance of DateTime.');
        }
    }
}
