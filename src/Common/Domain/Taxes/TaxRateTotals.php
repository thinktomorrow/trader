...
//    }
    ...
    private function groupTotalsByRate(): array
    {
        ...
        foreach ($this->taxables as $taxable) {
            $key = (string) $taxable->taxRate()->asPercent();

            if (!isset($totalsPerRate[$key])) {
                $totalsPerRate[$key] = ['percent' => $taxable->taxRate(), 'total' => Cash::zero()];
            }

            $totalsPerRate[$key]['total'] = $totalsPerRate[$key]['total']->add($taxable->taxableTotal());
        }
