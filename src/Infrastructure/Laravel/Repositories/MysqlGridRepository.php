...
//            ->groupBy(static::$variantTable . '.variant_id', 'product_data', 'product_order_column', 'variant_order_column')
    ...
    public function filterByTaxonIds(array $taxonIds, bool $already_grouped = false): static
    {
        $taxonIdsGroupedByTaxonomy = $already_grouped ? $taxonIds : $this->flattenedTaxonIds->getGroupedByTaxonomyByIds($taxonIds);

        // No taxon ids means no results
        if (count($taxonIdsGroupedByTaxonomy) === 0) {
            $this->builder->whereRaw('1 = 0');

            return $this;
        }

        $this->builder->where(function ($outer) use ($taxonIdsGroupedByTaxonomy) {
    ...
    public function getResults(?int $total = null): LengthAwarePaginator
    {
        ...
        $variantKeys = $this->getVariantKeysInBulk($results);

        return $results->setCollection(
            $results->getCollection()
                ->map(fn ($state) => get_object_vars($state))
                ->map(function (array $state) use ($productTaxa, $variantTaxa, $variantKeys) {
    ...
    protected function addDefaultSorting(Builder $builder): void
    {
    ...
    protected function getVariantKeysInBulk(Paginator $results): array
    {
        ...
        $variantKeyStates = DB::table(static::$variantKeysTable)
    ...
    protected function whereJsonLike($value)
    {
        ...
        $this->builder->where(function ($builder) use ($value, $keys, $locale) {
        ...
        //            foreach ([static::$productTable, static::$variantTable] as $table) {
        //                foreach ($keys[$table] as $key) {
        //                    $builder->orWhereRaw('LOWER(JSON_UNQUOTE(json_extract(`' . $table . '`.`data`, "$.' . $key . '"))) LIKE ?', '%' . $value . '%');
        //                }
        //            }
        //        });
    }
